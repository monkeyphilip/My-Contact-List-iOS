...
//  My Contact List
//
//  Created by Byung Lee on 4/30/19.
...
class LocationDemoViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func locationManager(_ mangaer: CLLocationManager,
    ...
    @IBAction func addressToCoordinates(_ sender: Any) {
        ...
        geoCoder.geocodeAddressString(address) {(placemarks, error) in
            self.processAddressResonse(withPlacemarks: placemarks, error: error)
        }
    }
    
    private func processAddressResonse(withPlacemarks placemarks: [CLPlacemark]?, error: Error?) {
        if let error = error {
